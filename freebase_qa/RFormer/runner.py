...
from load_datasets import load_dataset, get_dataset_ans
from modeling_rrag import RRAGLlamaForCausalLM, RRAGLlamaConfig
from trainer import RRAGTrainer
from metrics import evaluation_from_list
...
class RRAGRunner:
    ...
    def load_tokenizer(self):
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            self.model_name,
            trust_remote_code=True  # Qwen 仓库常需要
        )
        tokenizer.padding_side = "left"
        tokenizer.pad_token = tokenizer.eos_token

        # Qwen的分词器默认情况下不会将字符串"<unk>"映射为ID 0，因此需要手动设置，分三个步骤：
        # 1. 检查分词器默认的 unktoken 和 unk_token_id。默认为None
        # print('Default unk_token:', tokenizer.unk_token)
        # print('Default unk_token_id:', tokenizer.unk_token_id)

        # 2. 手动设置 unk_token 和 并同步 ID，确保 RRAGRunner.UNK_TOKEN ('<unk>') 被正确处理
        tokenizer.add_special_tokens({"unk_token": RRAGRunner.UNK_TOKEN})

        # 3. 从分词器获取真实的 unk_token_id 并更新全局设置
        #    这将确保 RRAGRunner.UNK_TOKEN_ID 被正确设置为分词器中 '<unk>' 的 ID
        RRAGRunner.set_unk_token_id(tokenizer.unk_token_id)
        print(f'Set tokenizer unk_token to {tokenizer.unk_token} with ID {tokenizer.unk_token_id}')

        self.tokenizer = tokenizer

    def load_dataset(self):
        if self.dataset_name not in ['webqsp', 'cwq', 'grailqa']:
            raise ValueError(self.dataset_name)
        self.instruction_dataset_train, self.instruction_dataset_test = load_dataset(self.input_path, self.max_prompt_length, self.tokenizer, retrieval_aware=self.retrieval_aware, RETRIEVAL_TOKEN=self.RETRIEVAL_TOKEN)
    ...
    def load_model(self):
        ...
        if self.use_rrag:
            config = RRAGLlamaConfig(
                ...
                unk_token=RRAGRunner.UNK_TOKEN,
                unk_token_id=RRAGRunner.UNK_TOKEN_ID,
                freeze_llm=self.freeze_llm,
                num_k=self.num_k,
                )
            if self.load_from_pretrained:
                print(f'load_from_pretrained: {self.pretrained_model_name}')
                self.model = RRAGLlamaForCausalLM.from_pretrained(
    ...
    def eval(self):
        ...
        if self.save_results:
            ...
            with open(pkl_save_path, 'wb') as f:
                pickle.dump(res, f)
                f.close()
        gt_ans = get_dataset_ans(self.instruction_dataset_test)
...
def main(dataset_name, train_data_path, test_data_path, **args):
    if dataset_name in ['webqsp', 'cwq', 'grailqa']:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default='webqsp', choices=['webqsp', 'cwq', 'grailqa'], help='Name of the dataset')
    parser.add_argument('--train_data_path', type=str, default='/media/xujiasheng/rag_projects/RAGE/data/webqsp/webqsp.train.pkl', help='Path for the hotpotqa, musique, 2wiki')
    parser.add_argument('--test_data_path', type=str, default='/media/xujiasheng/rag_projects/RAGE/data/webqsp/webqsp.test.pkl', help='Path for the hotpotqa, musique, 2wiki')
    parser.add_argument('--max_prompt_length', type=int, default=4096, help='Maximum prompt length')

    parser.add_argument('--model_name', type=str, required=False, default='Qwen/Qwen3-8B', help='Name of LLM')
    parser.add_argument('--load_in_8bit', action='store_true', default=False, help='Load in 8-bit precision')
    parser.add_argument('--save_model', action='store_true', default=True, help='If set, the trained model will be saved to outputdir')
    parser.add_argument('--output_dir', type=str, required=False, default='../model', help='Directory to save model')

    parser.add_argument('--use_rrag', action='store_true', default=True, help='Whether to use RRAG or not')
    ...
    parser.add_argument('--use_training', action='store_true', default=False, help='Use for training')
    parser.add_argument('--freeze_llm', action='store_true', default=True, help='Freeze LLM')
    parser.add_argument('--load_from_pretrained', action='store_true', default=True, help='Load from RRAG pretrained model')
    parser.add_argument('--pretrained_model_name', type=str, required=False, default='../model', help='Name of RRAG pretrained model')
    parser.add_argument('--use_lora', action='store_true', default=False, help='Use LoRA')
    parser.add_argument('--num_train_epochs', type=int, default=2, help='Number of training epochs')
    parser.add_argument('--per_device_train_batch_size', type=int, default=2, help='Batch size per device')

    parser.add_argument('--use_evaluation', action='store_true', default=True, help='Use for evaluation')
    parser.add_argument('--max_new_tokens', type=int, default=100, help='Maximum new tokens')
    parser.add_argument('--use_beam', action='store_true', default=False, help='Use beam search')
